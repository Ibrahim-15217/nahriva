...
class SubmitReportScreenFromEcoLens extends ConsumerStatefulWidget {
  const SubmitReportScreenFromEcoLens({super.key});

  @override
  ConsumerState<SubmitReportScreenFromEcoLens> createState() => _SubmitReportScreenFromEcoLensState();
}

class _SubmitReportScreenFromEcoLensState extends ConsumerState<SubmitReportScreenFromEcoLens> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      String photoUrl = '';
      if (_photoPath != null) {
        final repo = ref.read(reportRemoteDataSourceProvider);
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              Text('Photo', style: AppTypography.title.copyWith(color: AppColors.textPrimary)),
              const SizedBox(height: 8),
              PhotoPicker(onPhotoPicked: (path) => _photoPath = path),
              const SizedBox(height: 20),
              Text('Waste Type (from EcoLens)', style: AppTypography.title.copyWith(color: AppColors.textPrimary)),
              const SizedBox(height: 8),
              ...
              Row(
                children: [
                  ...
                  Text(
                    _isLocating ? 'Detecting location...' : _currentPosition != null ? 'Location detected' : 'Location unavailable',
